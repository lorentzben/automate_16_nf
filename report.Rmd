...
if(!require(tidyverse)){renv::install("tidyverse")}
if(!require(ampvis2)){renv::install("MadsAlbertsen/ampvis2")}
if(!require(devtools)) {renv::install("devtools")}
if(!require(ggdendro)){renv::install("ggendro")}
...
#if(!require(pegas)){renv::install("pegas")}

if(!require(ggvegan)){renv::install("gavinsimpson/ggvegan")}
if(!require(ggpubr)){renv::install("ggpubr")}
if(!require(dplyr)){renv::install("dplyr")}
if(!require(jamba)){renv::install("jmw86069/jamba")}
if(!require(ggConvexHull)){renv::install("cmartin/ggConvexHull")}
...
simpson_vals[[ioi]] <- factor(simpson_vals[[ioi]],levels=ioi_order)
...
perma_res <- PERMANOVA::PerMANOVA.Simple(D = as.matrix(u_unifrac), grupo = cycle_1@sam_data[[ioi]])

print(data.frame(data.frame(res$Inicial),res$pval))

# Pairwise PERMANOVA of Unweighted Unifrac
perma_pairwise <- pairwise_permanova_uni(cycle_1, ioi, F)
kable(test) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```
...
perma_res <- PERMANOVA::PerMANOVA.Simple(D = as.matrix(distance(cycle_1, 'bray',)) , grupo = cycle_1@sam_data[[ioi]])
print(data.frame(data.frame(res$Inicial),res$pval))
...
perma_res <- PERMANOVA::PerMANOVA.Simple(D = as.matrix(distance(cycle_1, 'jaccard',)) , grupo = cycle_1@sam_data[[ioi]])
print(data.frame(data.frame(res$Inicial),res$pval))
